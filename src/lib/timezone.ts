/**
 * Конвертація між UTC (як зберігає домен) і київським «настінним» часом.
 *
 * Спільний модуль (чистий, через Intl): використовують і Cliniccards-адаптер
 * (Kyiv↔UTC для API), і mock (автор зміни в київських годинах), і UI (показ часу
 * та групування за київською годиною). Працює і на сервері, і в клієнті.
 */
...
export function kyivWallToUtcIso(wall: string): string {
  ...
  if (!m) throw new Error(`Некоректний формат настінного часу: ${wall}`);
...
/** Київська година (0–23) моменту. */
export function kyivHour(iso: string): number {
  return zonedParts(new Date(iso)).hour;
}

/** Київська дата "YYYY-MM-DD" моменту. */
export function kyivDate(iso: string): string {
  return utcIsoToKyivParts(iso).date;
}
